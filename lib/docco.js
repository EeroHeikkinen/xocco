(function() {
  ...
  parse = function(source, code) {
    ...
    for (_i = 0, _len = lines.length; _i < _len; _i++) {
      line = lines[_i];
	  // Remove byte-order mark if present, it will screw up things
      ...
      if (line.match(language.multi_start_matcher) || in_multi) {
        ...
        if (line.match(language.multi_end_matcher)) {
          ...
          try {
            ...
            docs_text = multi_accum.replace(language.multi_start, '').replace(language.multi_end, '').replace(/^(\s*)((\S+\s*?)*)(\s*)$/,"$2");
          }
          multi_accum = '';
        }
  ...
  for (ext in languages) {
	...
	else if(l.multi_start && l.multi_end) {
		l.divider_text = '' + l.multi_start + 'DIVIDER' + l.multi_end + '';
		l.divider_html = new RegExp('<span class="c1?">' + l.multi_start.replace(/</g, '&lt;').replace(/>/g, '&gt;') + 'DIVIDER' + l.multi_end.replace(/</g, '&lt;').replace(/>/g, '&gt;') + '<\\/span>');
	}
	else console.log("No symbol provided for language");
  ...
  destination = function(filepath, context) {
	...
	var trunc = "", filename = filepath;
	if(filepath.lastIndexOf('\\') != -1) {
		console.log("windows!");
		trunc = filepath.substring(0, filepath.lastIndexOf('\\')+1);
    ...
    return trunc + ("" + context.config.output_dir + "/") + filename + '.html';
  };
  ...
  docco_template = jade.compile(fs.readFileSync(__dirname + '/../resources/docco.jade', 'utf8').toString(), {
  ...
  parse_args = function(callback) {
    ...
    return callback([roots], project_name, args);
  };
  ...
  check_config = function(context, pkg) {
    ...
    return context.config = _.extend(defaults, pkg.x || {});
  };
